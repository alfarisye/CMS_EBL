...
<main id="main" class="main">
    <div id="TSalMasterCoa" class="d-none">
        <div class="pagetitle">
            <h1>MASTER COA</h1>
            <nav>
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active"><a href="<?= site_url("master/master-coa") ?>">MASTER COA</a></li>
                </ol>
            </nav>
        </div>
               ...
               <form action="" @submit.prevent='showInsert?insertData():updateData()'>
                <table>
                    <tr>
                        ...
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="sequence" name="sequence" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="sequence ..." v-model="vdata['sequence']" >
                        </td>
                    </tr>
                    <tr>
                        <td class="p-2">Coa</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="coa" name="coa" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="coa ..." v-model="vdata['coa']" >
                        </td>
                    </tr>
                    <tr>
                        <td class="p-2">Min input</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="min" name="min" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="min ..." v-model="vdata['min']" >
                        </td>
                    </tr>
                    <tr>
                        <td class="p-2">Max input</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="max" name="max" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="max ..." v-model="vdata['max']" >
                        </td>
                    </tr>
                </table>
                <hr class="my-4">
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >MASTER COA</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-3">
                         <form v-if="datanya.length>0" action="<?= site_url() ?>/production/report/download?nama_file=T_SAL_MASTER_COA" method="post">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                               <td class="text-xs">{{item.sequence}}</td>
                                <td class="text-xs">{{item.coa}}</td>
                                <td class="text-xs">{{item.status=='X'?'Tidak Aktiv':'Aktif'}}</td>
                                <td>
    ...
    new Vue({
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/t-sal-master-coa`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                ...
                axios.put("<?= site_url() ?>" + `/api/t-sal-master-coa/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/t-sal-master-coa/${data.id}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                this.datanya=[]
                let data = await axios.get("<?= site_url() ?>" + `/api/get/t-sal-master-coa`);
        ...
        mounted() {
            this.getData();
            document.getElementById('TSalMasterCoa').classList.remove('d-none');
            this.$forceUpdate();
        },
    })
</script>
