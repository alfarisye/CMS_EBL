...
<main id="main" class="main">
    <div id="masterLaytime" class="d-none">
        <div class="pagetitle">
            <h1>Master Laytime</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= site_url("/") ?>">Home</a></li>
                    <li class="breadcrumb-item">Sales</li>
                    <li class="breadcrumb-item active"><a href="<?= site_url("master/master-laytime") ?>">Master Laytime</a></li>
                </ol>
            </nav>
        </div>
               ...
               <form action="" @submit.prevent='showInsert?insertData():updateData()'>
                <table>
                    <tr>
                        ...
                        <td class="px-2">:</td>
                        <td>
                            <input required :disabled="!showInsert" type="text" id="code" name="code" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="code ..." v-model="vdata['code']" >
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            <div class="sm-form">
                            <textarea type="text" id="description" name="description" rows="2" placeholder="description..." class="form-control md-textarea" v-model="vdata.description" ></textarea>
                            </div>
                        </td>
                    </tr>
                    <tr>
                        <td class="p-2">Crane Rate %</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="rate" name="rate" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="rate ..." v-model="vdata['rate']" >
                        </td>
                    </tr>
                </table>
                <hr class="my-4">
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >Master Laytime</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                                ...
                                <td>
                                    {{item.status!='X'?'Active':'Non Active'}}
                                </td>
                                <td>
    ...
    new Vue({
        ...
        data(){
            return{
                ...
                target:{},
                disableInput:[],
                sortTable:['id','code','description','rate','status'], // disusun berdasarkan urutan td td td
                modals:false,
                showInsert:false,
                // CUSTOM
                datanya:[],
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/master-laytime`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                ...
                axios.put("<?= site_url() ?>" + `/api/master-laytime/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/master-laytime/${data.id}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                let data;
                this.datanya=[]
                data = await axios.get("<?= site_url() ?>" + `/api/get/master-laytime`);
        ...
        mounted() {
            this.getData();
            document.getElementById('masterLaytime').classList.remove('d-none');
