...
<main id="main" class="main">
    <div id="MdAnnualdisob" class="d-none">
        <div class="pagetitle">
            <h1>Annual Budget OB Distance</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= site_url("/") ?>">Home</a></li>
                    <li class="breadcrumb-item">Master</li>
                    <li class="breadcrumb-item active"><a href="<?= site_url("budget/annualdisob") ?>">Annual Budget OB Distance</a></li>
                </ol>
            </nav>
        </div>
        ...
        <tr>
                    <td class="p-2">Annual Budget OB Distance</td>
                    <td class="px-2">:</td>
                    <td>
                        <input required  type="text" id="disob_annualbudget_qty" name="disob_annualbudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="disob_annualbudget_qty ..." v-model="vdata['disob_annualbudget_qty']" >
                    </td>
                </tr>
                <tr>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >Annual Budget OB Distance</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-3">
                         <form v-if="datanya.length>0" action="<?= site_url() ?>/production/report/download?nama_file=md_annualdisob" method="post">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                               <td class="text-xs">{{item.id_annualdisob}}</td>
                               <td class="text-xs">{{item.project}}</td>
                               <td class="text-xs">{{item.year}}</td>
                                <td class="text-xs">{{item.disob_annualbudget_qty}}</td>
    ...
    new Vue({
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/md-annualdisob`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                if(!this.validateRequired())return;
                sdb.loadingOn();
                let id=this.vdata.id_annualdisob;
                delete this.vdata.id_annualdisob;
                this.vdata.revision=parseInt(this.vdata.revision)+1;
                axios.put("<?= site_url() ?>" + `/api/md-annualdisob/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/md-annualdisob/${data.id_annualdisob}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                this.datanya=[]
                let data = await axios.get("<?= site_url() ?>" + `/api/get/md-annualdisob`);
        ...
        mounted() {
            this.getData();
            document.getElementById('MdAnnualdisob').classList.remove('d-none');
            this.$forceUpdate();
        },
    })
</script>
