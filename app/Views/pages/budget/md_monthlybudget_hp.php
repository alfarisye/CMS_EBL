...
<main id="main" class="main">
    <div id="MdMonthlybudgetHp" class="d-none">
        <div class="pagetitle">
            <h1>Monthly Budget HP</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= site_url("/") ?>">Home</a></li>
                    <li class="breadcrumb-item">Master</li>
                    <li class="breadcrumb-item active"><a href="<?= site_url("budget/md-monthlybudget-hp") ?>">Monthly Budget HP</a></li>
                </ol>
            </nav>
        </div>
        ...
        <tr>
                    <td class="p-2">Monthly Budget Hauling to Port</td>
                    <td class="px-2">:</td>
                    <td>
                        <input required  type="text" id="hp_mounthlybudget_qty" name="hp_mounthlybudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="hp_mounthlybudget_qty ..." v-model="vdata['hp_mounthlybudget_qty']" >
                    </td>
                </tr>
        <tr>
                    <td class="p-2">Dayly Budget Haulingto Port</td>
                    <td class="px-2">:</td>
                    <td>
                        <input required  type="text" id="hp_dailybudget_qty" name="hp_dailybudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="hp_dailybudget_qty ..." v-model="vdata['hp_dailybudget_qty']" >
                    </td>
                </tr>
                <tr>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >Monthly Budget HP</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-3">
                         <form v-if="datanya.length>0" action="<?= site_url() ?>/production/report/download?nama_file=md_monthlybudget_hp" method="post">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        <tr>
                           <th class="text-xs" style="background:lightgreen;" scope="col">
                            ...
                            <th class="text-xs" style="background:lightgreen;" scope="col">
                                Monthly Budget Hauling to Port QTY &#8593;&#8595;
                            </th>
                            <th class="text-xs" style="background:lightgreen;" scope="col">
                                Daily Budget Hauling to Port QTY &#8593;&#8595;
                            </th>
                            <th class="text-xs" style="background:lightgreen;" scope="col">
                                Revision &#8593;&#8595;
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                                    <td class="text-xs">{{item.id_monthlybudgethp}}</td>
                                    <td class="text-xs">{{item.project}}</td>
                                    <td class="text-xs">{{item.year}}</td>
                                    <td class="text-xs">{{item.month}}</td>
                                    <td class="text-xs">{{item.hp_mounthlybudget_qty}}</td>
                                    <td class="text-xs">{{item.hp_dailybudget_qty}}</td>
    ...
    new Vue({
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/md-monthlybudget-hp`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                if(!this.validateRequired())return;
                sdb.loadingOn();
                let id=this.vdata.id_monthlybudgethp;
                delete this.vdata.id_monthlybudgethp;
                this.vdata.revision=parseInt(this.vdata.revision)+1;
                
                axios.put("<?= site_url() ?>" + `/api/md-monthlybudget-hp/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/md-monthlybudget-hp/${data.id_monthlybudgethp}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                this.datanya=[]
                let data = await axios.get("<?= site_url() ?>" + `/api/get/md-monthlybudget-hp`);
        ...
        mounted() {
            this.getData();
            document.getElementById('MdMonthlybudgetHp').classList.remove('d-none');
            this.$forceUpdate();
        },
    })
</script>
