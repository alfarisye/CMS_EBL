...
<main id="main" class="main">
    <div id="MdMonthlyDiscg" class="d-none">
        <div class="pagetitle">
            <h1>Monthly Budget CG Distance</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= site_url("/") ?>">Home</a></li>
                    <li class="breadcrumb-item">Master</li>
                    <li class="breadcrumb-item active"><a href="<?= site_url("budget/md-monthly-discg") ?>">Monthly Budget CG Distance</a></li>
                </ol>
            </nav>
        </div>
        ...
        <tr>
                    <td class="p-2">Monthly Budget CG Distance QTY</td>
                    <td class="px-2">:</td>
                    <td>
                        <input required  type="text" id="discg_monthlybudget_qty" name="discg_monthlybudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="discg_monthlybudget_qty ..." v-model="vdata['discg_monthlybudget_qty']" >
                    </td>
                </tr>
        <tr>
                    <td class="p-2">Daily Budget CG Distance QTY</td>
                    <td class="px-2">:</td>
                    <td>
                        <input required  type="text" id="discg_dailybudget_qty" name="discg_dailybudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="discg_dailybudget_qty ..." v-model="vdata['discg_dailybudget_qty']" >
                    </td>
                </tr>
                <tr>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >Monthly Budget CG Distance</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-3">
                         <form v-if="datanya.length>0" action="<?= site_url() ?>/production/report/download?nama_file=md_monthly_discg" method="post">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                                <td class="text-xs">{{item.id_monthlybudget_discg}}</td>
                                <td class="text-xs">{{item.project}}</td>
                                <td class="text-xs">{{item.year}}</td>
                                <td class="text-xs">{{item.month}}</td>
                                <td class="text-xs">{{item.discg_monthlybudget_qty}}</td>
                                <td class="text-xs">{{item.discg_dailybudget_qty}}</td>
    ...
    new Vue({
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/md-monthly-discg`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                if(!this.validateRequired())return;
                sdb.loadingOn();
                let id=this.vdata.id_monthlybudget_discg;
                delete this.vdata.id_monthlybudget_discg;
                this.vdata.revision=parseInt(this.vdata.revision)+1;
                
                axios.put("<?= site_url() ?>" + `/api/md-monthly-discg/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/md-monthly-discg/${data.id_monthlybudget_discg}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                this.datanya=[]
                let data = await axios.get("<?= site_url() ?>" + `/api/get/md-monthly-discg`);
        ...
        mounted() {
            this.getData();
            document.getElementById('MdMonthlyDiscg').classList.remove('d-none');
            this.$forceUpdate();
        },
    })
</script>
