...
<main id="main" class="main">
    <div id="MdMonthlybudgetCc" class="d-none">
        <div class="pagetitle">
            <h1>Annual Budget CC</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= site_url("/") ?>">Home</a></li>
                    <li class="breadcrumb-item">Master</li>
                    <li class="breadcrumb-item active"><a href="<?= site_url("budget/monthlybudget-cc") ?>">Annual Budget CC</a></li>
                </ol>
            </nav>
        </div>
               ...
               <form action="" @submit.prevent='showInsert?insertData():updateData()'>
                <table>
                    ...
                    <tr>
                        ...
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="month" name="month" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="month ..." v-model="vdata['month']" >
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="p-2">Monthly Budget Crush Coal</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="cc_mounthlybudget_qty" name="cc_mounthlybudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="cc_mounthlybudget_qty ..." v-model="vdata['cc_mounthlybudget_qty']" >
                        </td>
                    </tr>
                    <tr>
                        <td class="p-2">Daily Budget Crush Coal</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="cc_dailybudget_qty" name="cc_dailybudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="cc_dailybudget_qty ..." v-model="vdata['cc_dailybudget_qty']" >
                        </td>
                    </tr>
                    <tr>
                        <td class="p-2">Revision</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="revision" name="revision" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="revision ..." v-model="vdata['revision']" >
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        <td class="p-2">Contractor</td>
                        <td class="px-2">:</td>
                        <td>
                            <select class='form-control' v-model="vdata.id_contractor">
                                <option :value="item.id" v-for="(item, index) in contractor" :key="index+'contractor'">{{item.contractor_name}}</option>
                            </select>
                        </td>
                    </tr>
                </table>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >Annual Budget CC</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-3">
                         <form v-if="datanya.length>0" action="<?= site_url() ?>/production/report/download?nama_file=md_monthlybudget_cc" method="post">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        <tr>
                           <th class="text-xs" style="background:lightgreen;" scope="col">
                            ...
                            <th class="text-xs" style="background:lightgreen;" scope="col">
                                Monthly Budget Crush Coal QTY &#8593;&#8595;
                            </th>
                            <th class="text-xs" style="background:lightgreen;" scope="col">
                                Daily Budget Crush Coal QTY&#8593;&#8595;
                            </th>
                            <th class="text-xs" style="background:lightgreen;" scope="col">
                                Revision &#8593;&#8595;
                            </th>
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                                <td class="text-xs">{{item.id_monthlybudgetcc}}</td>
                                <td class="text-xs">{{item.project}}</td>
                                <td class="text-xs">{{item.year}}</td>
                                <td class="text-xs">{{item.month}}</td>
                                <td class="text-xs">{{item.cc_mounthlybudget_qty}}</td>
                                <td class="text-xs">{{item.cc_dailybudget_qty}}</td>
    ...
    new Vue({
        ...
        data(){
            return{
                ...
                target:{},
                disableInput:[],
                sortTable:["id_contractor"], // disusun berdasarkan urutan td td td
                modals:false,
                showInsert:false,
                contractor:[],
                // CUSTOM
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/md-monthlybudget-cc`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                if(!this.validateRequired())return;
                sdb.loadingOn();
                let id=this.vdata.id_monthlybudgetcc;
                delete this.vdata.id_monthlybudgetcc;
                this.vdata.revision=parseInt(this.vdata.revision)+1;
                axios.put("<?= site_url() ?>" + `/api/md-monthlybudget-cc/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/md-monthlybudget-cc/${data.id_monthlybudgetcc}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                this.datanya=[]
                let data = await axios.get("<?= site_url() ?>" + `/api/get/md-monthlybudget-cc`);
                this.datanya=data.data;
                let res = await axios.get("<?= site_url() ?>" + `/api/get/md-contractors`);
        ...
        mounted() {
            this.getData();
            document.getElementById('MdMonthlybudgetCc').classList.remove('d-none');
            this.$forceUpdate();
        },
    })
</script>
