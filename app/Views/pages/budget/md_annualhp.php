...
<main id="main" class="main">
    <div id="MdAnnualhp" class="d-none">
        <div class="pagetitle">
            <h1>Annual Budget HP</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= site_url("/") ?>">Home</a></li>
                    <li class="breadcrumb-item">Master</li>
                    <li class="breadcrumb-item active"><a href="<?= site_url("budget/md-annualhp") ?>">Annual Budget HP</a></li>
                </ol>
            </nav>
        </div>
            ...
            <div class="rounded-lg shadow p-3 bg-white animate__animated animate__bounceIn">
               <form action="" @submit.prevent='showInsert?insertData():updateData()'>
                <table>
                    ...
                    <tr>
                        <td class="p-2">Annual Budget Hauling to Port</td>
                        <td class="px-2">:</td>
                        <td>
                            <input required  type="text" id="hp_annualbudget_qty" name="hp_annualbudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="hp_annualbudget_qty ..." v-model="vdata['hp_annualbudget_qty']" >
                        </td>
                    </tr>
                    <tr>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >Annual Budget HP</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-3">
                         <form v-if="datanya.length>0" action="<?= site_url() ?>/production/report/download?nama_file=md_annualhp" method="post">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                               <td class="text-xs">{{item.id_annualhp}}</td>
                                <td class="text-xs">{{item.year}}</td>
                                <td class="text-xs">{{item.project}}</td>
                                <td class="text-xs">{{item.hp_annualbudget_qty}}</td>
    ...
    new Vue({
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/md-annualhp`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                if(!this.validateRequired())return;
                sdb.loadingOn();
                let id=this.vdata.id_annualhp;
                delete this.vdata.id_annualhp;
                this.vdata.revision=parseInt(this.vdata.revision)+1;
                axios.put("<?= site_url() ?>" + `/api/md-annualhp/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/md-annualhp/${data.id_annualhp}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                this.datanya=[]
                let data = await axios.get("<?= site_url() ?>" + `/api/get/md-annualhp`);
        ...
        mounted() {
            this.getData();
            document.getElementById('MdAnnualhp').classList.remove('d-none');
            this.$forceUpdate();
        },
    })
</script>
