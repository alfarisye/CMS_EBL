...
<main id="main" class="main">
    <div id="MdAnnualdiscg" class="d-none">
        <div class="pagetitle">
            <h1>Annual Budget CG Distance</h1>
            <nav>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="<?= site_url("/") ?>">Home</a></li>
                    <li class="breadcrumb-item">Master</li>
                    <li class="breadcrumb-item active"><a href="<?= site_url("budget/md-annualdiscg") ?>">Annual Budget CG Distance</a></li>
                </ol>
            </nav>
        </div>
        ...
        <tr>
                    <td class="p-2">Annual Budget CG Distance</td>
                    <td class="px-2">:</td>
                    <td>
                        <input required  type="text" id="discg_annualbudget_qty" name="discg_annualbudget_qty" style="width:60%;" class="form-control p-1  rounded-sm shadow-sm" placeholder="discg_annualbudget_qty ..." v-model="vdata['discg_annualbudget_qty']" >
                    </td>
                </tr>
                <tr>
        ...
        <div class="card">
            <div class="card-body">
                <h5 class="card-title" >Annual Budget CG Distance</h5>
                <div class="row py-2" >
                    <div class="col-sm-3">
                    ...
                    <div class="col-sm-3">
                         <form v-if="datanya.length>0" action="<?= site_url() ?>/production/report/download?nama_file=md_annualdiscg" method="post">
                    ...
                    <table class="table table-sm table-bordered table-striped">
                        ...
                        <tbody v-if="datanya.length>0">
                            <tr v-for="(item, index) in td" :key="index+'form'" >
                               <td class="text-xs">{{item.id_annualdiscg}}</td>
                                <td class="text-xs">{{item.year}}</td>
                                <td class="text-xs">{{item.project}}</td>
                                <td class="text-xs">{{item.discg_annualbudget_qty}}</td>
    ...
    new Vue({
        ...
        methods: {
            ...
            async insertData(){
                ...
                axios.post("<?= site_url() ?>" + `/api/md-annualdiscg`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async updateData(){
                if(!this.validateRequired())return;
                sdb.loadingOn();
                let id=this.vdata.id_annualdiscg;
                delete this.vdata.id_annualdiscg;
                this.vdata.revision=parseInt(this.vdata.revision)+1;
                axios.put("<?= site_url() ?>" + `/api/md-annualdiscg/${id}`,this.vdata).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
                    this.getData()
            ...
            async deleteData(data){
                ...
                sdb.loadingOn();
                axios.delete("<?= site_url() ?>" + `/api/md-annualdiscg/${data.id_annualdiscg}`).then(res=>{
                    sdb.loadingOff();
                    this.modals=false;
            ...
            async getData(){
                this.datanya=[]
                let data = await axios.get("<?= site_url() ?>" + `/api/get/md-annualdiscg`);
        ...
        mounted() {
            this.getData();
            document.getElementById('MdAnnualdiscg').classList.remove('d-none');
            this.$forceUpdate();
        },
    })
</script>
