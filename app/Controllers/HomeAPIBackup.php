...
class HomeAPI extends BaseController
{
    ...
    public function index()
    {
        ...
        if ($type == 'yearly') {
            ...
            $cost_minings = $db->query("SELECT year, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            ...
            GROUP BY year, id_contractor")->getResultArray();
            $grouped_cost_minings = array();
            foreach ($cost_minings as $c) {
                $grouped_cost_minings[$c['contractor']][$c['year']] = $c['total_cost'];
            }
            $data['cost_minings'] = $grouped_cost_minings;
            //Cost Contractor - mobile yearly
            $cost_overburden = $db->query("SELECT year, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE tc.id_costtype = 11
            GROUP BY year, id_contractor")->getResultArray();
            $grouped_cost_overburden = array();
            foreach ($cost_overburden as $c) {
                $grouped_cost_overburden[$c['contractor']][$c['year']] = $c['total_cost'];
            }
            $data['cost_overburden'] = $grouped_cost_overburden;

            $cost_coal_getting = $db->query("SELECT year, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE tc.id_costtype = 12
            GROUP BY year, id_contractor")->getResultArray();
            $grouped_cost_coal_getting = array();
            foreach ($cost_coal_getting as $c) {
                $grouped_cost_coal_getting[$c['contractor']][$c['year']] = $c['total_cost'];
            }
            $data['cost_coal_getting'] = $grouped_cost_coal_getting;

            $cost_hauling = $db->query("SELECT year, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE tc.id_costtype = 13
            GROUP BY year, id_contractor")->getResultArray();
            $grouped_cost_hauling = array();
            foreach ($cost_hauling as $c) {
                $grouped_cost_hauling[$c['contractor']][$c['year']] = $c['total_cost'];
            }
            $data['cost_hauling'] = $grouped_cost_hauling;

            $cost_barge = $db->query("SELECT year, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE tc.id_costtype = 14
            GROUP BY year, id_contractor")->getResultArray();
            $grouped_cost_barge = array();
            foreach ($cost_barge as $c) {
                $grouped_cost_barge[$c['contractor']][$c['year']] = $c['total_cost'];
            }
            $data['cost_barge'] = $grouped_cost_barge;

            $cost_crush = $db->query("SELECT year, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE tc.id_costtype = 15
            GROUP BY year, id_contractor")->getResultArray();
            $grouped_cost_crush = array();
            foreach ($cost_crush as $c) {
                $grouped_cost_crush[$c['contractor']][$c['year']] = $c['total_cost'];
            }
            $data['cost_crush'] = $grouped_cost_crush;
            ...
            // $inventory_closing_value = $db->query("SELECT isc.stock AS crusher_stock, isp.stock AS port_stock, isr.stock AS rom_stock, DAY(isc.posting_date) AS `day`, MONTH(isc.posting_date) AS bulan, YEAR(isc.posting_date) AS tahun  
            // FROM im_stock_cc isc INNER JOIN im_stock_port isp ON isp.posting_date = isc.posting_date 
            // INNER JOIN im_stock_raw isr ON isr.posting_date = isc.posting_date
            // WHERE DAY(isc.posting_date) = 25 ORDER BY bulan")->getResultArray();
            $inventory_closing_value = $db->query("SELECT SUM(isc.stock) AS crusher_stock, SUM(isp.stock) AS port_stock, SUM(isr.stock) AS rom_stock, YEAR(isc.posting_date) AS tahun  
            FROM im_stock_cc isc INNER JOIN im_stock_port isp ON isp.posting_date = isc.posting_date 
            INNER JOIN im_stock_raw isr ON isr.posting_date = isc.posting_date
            WHERE DAY(isc.posting_date) = 25
            GROUP BY tahun")->getResultArray();
        ...
        } else { //sales & cost contracotr - monthly
            $SalesShipment = new SalesShipment();
            $builder_shipment = $SalesShipment->builder();
            $total_shipment = $builder_shipment->select("SUM(bl_qty) AS total,
            ...
            // Cost Contractor (IDR)  - OB - For mobile
            $cost_overburden = $db->query("SELECT month, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE year = $year AND tc.id_costtype = 11
            GROUP BY month, id_contractor")->getResultArray();
            $grouped_cost_overburden = array();
            foreach ($cost_overburden as $c) {
                $grouped_cost_overburden[$c['contractor']][$c['month']] = $c['total_cost'];
            }
            $data['cost_overburden'] = $grouped_cost_overburden;

            //cost contractor coal getting
            $cost_coal_getting = $db->query("SELECT month, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE year = $year AND tc.id_costtype = 12
            GROUP BY month, id_contractor")->getResultArray();
            $grouped_cost_coal_getting = array();
            foreach ($cost_coal_getting as $c) {
                $grouped_cost_coal_getting[$c['contractor']][$c['month']] = $c['total_cost'];
            }
            $data['cost_coal_getting'] = $grouped_cost_coal_getting;

            //cost contractor hauling
            $cost_hauling = $db->query("SELECT month, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE year = $year AND tc.id_costtype = 13
            GROUP BY month, id_contractor")->getResultArray();
            $grouped_cost_hauling = array();
            foreach ($cost_hauling as $c) {
                $grouped_cost_hauling[$c['contractor']][$c['month']] = $c['total_cost'];
            }
            $data['cost_hauling'] = $grouped_cost_hauling;

            //cost contractor barge
            $cost_barge = $db->query("SELECT month, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE year = $year AND tc.id_costtype = 14
            GROUP BY month, id_contractor")->getResultArray();
            $grouped_cost_barge = array();
            foreach ($cost_barge as $c) {
                $grouped_cost_barge[$c['contractor']][$c['month']] = $c['total_cost'];
            }
            $data['cost_barge'] = $grouped_cost_barge;

            //cost contractor crush coal
            $cost_crush = $db->query("SELECT month, SUM(cost) AS total_cost, contractor_name AS contractor FROM t_costmining tc 
            INNER JOIN md_costtype mc ON mc.id_costtype = tc.id_costtype
            INNER JOIN md_contractors mc2 ON mc2.id = tc.id_contractor 
            WHERE year = $year AND tc.id_costtype = 15
            GROUP BY month, id_contractor")->getResultArray();
            $grouped_cost_crush = array();
            foreach ($cost_crush as $c) {
                $grouped_cost_crush[$c['contractor']][$c['month']] = $c['total_cost'];
            }
            $data['cost_crush'] = $grouped_cost_crush;
        ...
        if ($type == 'yearly') { //filter by yearly
            ...
            COALESCE((SELECT SUM(B.ob_monthlybudget_qt) FROM md_monthlybudget B), 0) AS budget")->getRowArray();

            // SUM COAL GETTING
            $data['cg_production'] = $db->query("SELECT (SELECT SUM(A.qty)/1000 FROM T_Adjustment A WHERE A.transaksi = 'Coal Getting') AS actual, 
            COALESCE((SELECT SUM(B.cg_monthlybudget_qt) FROM md_monthlybudget B), 0) AS budget")->getRowArray();
            ...
            COALESCE((SELECT SUM(B.hp_mounthlybudget_qty) FROM md_monthlybudget_hp B WHERE B.status = '1'), 0) AS budget")->getRowArray();
            ...
            COALESCE((SELECT SUM(B1.ob_monthlybudget_qt) FROM md_monthlybudget B1), 0) AS budget_ob,
            (SELECT SUM(A2.qty)/1000 FROM T_Adjustment A2 WHERE A2.transaksi = 'Coal Getting') AS actual_cg,
            COALESCE((SELECT SUM(B2.cg_monthlybudget_qt) FROM md_monthlybudget B2), 0) AS budget_cg")->getRowArray();
            ...
            JOIN (
                SELECT SUM(B.ob_monthlybudget_qt) AS budget, B.year AS tahun2
            ...
            JOIN (
                SELECT SUM(B.cg_monthlybudget_qt) AS budget, B.year AS tahun2
                FROM md_monthlybudget B
                GROUP BY tahun2
            ) subquery2
            ON subquery1.tahun1 = subquery2.tahun2
            ...
            (SELECT SUM(budget.ob_monthlybudget_qt)
            ...
            $data['contractor_ob'] = $grouped_contractors; // yearly
            ...
            (SELECT SUM(budget.cg_monthlybudget_qt)
                            FROM md_monthlybudget budget
                            where budget.year = qmaster.bulan
            ...
            $data['contractor_cg'] = $grouped_contractorsC; // yearly
            ...
            $data['actual_distance'] = $db->query("WITH t_abc AS (
               ...
               (SELECT SUM(disob_monthlybudget_qty * mm.ob_monthlybudget_qt)/SUM(mm.ob_monthlybudget_qt) FROM md_monthly_disob mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` WHERE mmd.year =  t_abc.bulan) AS target_ob,
               (SELECT SUM(discg_monthlybudget_qty * mm.cg_monthlybudget_qt)/SUM(mm.cg_monthlybudget_qt) FROM md_monthly_discg mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` WHERE mmd.year = t_abc.bulan) AS target_cg
                FROM t_abc
                GROUP BY bulan 
            ...
            $contractor_distance = $db->query("WITH t_abc AS (
                SELECT
                YEAR AS bulan, MONTH AS Mybulan, id_contractor AS contractor, 
                SUM(CASE WHEN transaksi = 'Overburden' THEN qty ELSE 0 END) AS OB,
                SUM(CASE WHEN transaksi = 'Distance OB' THEN qty ELSE 0 END) AS DO,
                SUM(CASE WHEN transaksi = 'Coal Getting' THEN qty/1000 ELSE 0 END) AS CG,
                SUM(CASE WHEN transaksi = 'Distance CG' THEN qty ELSE 0 END) AS DC
                FROM T_Adjustment ta
					 WHERE ta.transaksi = 'Overburden' OR ta.transaksi = 'Distance OB' OR ta.transaksi = 'Coal Getting' OR ta.transaksi = 'Distance CG'
					 AND ta.id_contractor IS NOT NULL                                                                                                                                                                                                                  
                GROUP BY bulan, Mybulan, contractor
                ), t_abc_summary AS 
                (SELECT
					 t_abc.bulan, t_abc.Mybulan, t_abc.contractor AS contractor_id,
               (SELECT tctr.contractor_name from md_contractors tctr WHERE tctr.id = t_abc.contractor) AS contractor,
				SUM(OB) AS ob_total,
                SUM(CG) AS cg_total,
                SUM(OB * DO) AS ob,
                SUM(CG * DC) AS cg,
                (SELECT SUM(disob_monthlybudget_qty * mm.ob_monthlybudget_qt)/SUM(mm.ob_monthlybudget_qt) FROM md_monthly_disob mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` AND mm.id_contractor = mmd.id_contractor WHERE mmd.year =  t_abc.bulan AND mmd.month =  t_abc.Mybulan AND mmd.id_contractor = t_abc.contractor) AS target_ob,
                (SELECT SUM(discg_monthlybudget_qty * mm.cg_monthlybudget_qt)/SUM(mm.cg_monthlybudget_qt) FROM md_monthly_discg mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` AND mm.id_contractor = mmd.id_contractor WHERE mmd.year =  t_abc.bulan AND mmd.month = t_abc.Mybulan  AND mmd.id_contractor = t_abc.contractor) AS target_cg
                FROM t_abc
                WHERE contractor IS NOT NULL
                GROUP BY bulan, Mybulan, contractor_id
                ORDER BY contractor, bulan, Mybulan)
                SELECT SUM(ob) AS ob, SUM(ob_total) AS ob_total, SUM(cg) AS cg, SUM(cg_total) AS cg_total,
                SUM(target_ob * target_cg)/SUM(target_ob) AS target_ob,
                SUM(target_ob * target_cg)/SUM(target_cg) AS target_cg,
                bulan, contractor
                from t_abc_summary
                GROUP BY bulan, contractor_id")->getResultArray();
            ...
            $coals_index = $db->query("SELECT year_index AS tahun,
            SUM(CASE WHEN index_type = 'MV' THEN index_qty ELSE NULL END) AS mv,
            (SELECT SUM(index_qty) FROM T_CoalIndex WHERE year_index = tahun AND index_type = 'Ici420' AND deletion_status = 0 ) as ic_total,
            (SELECT SUM(index_qty) FROM T_CoalIndex WHERE year_index = tahun AND index_type = 'EBL' AND deletion_status = 0 ) as newc_total
            FROM T_CoalIndex
            WHERE  deletion_status = 0 AND year_index IS NOT NULL
            GROUP BY tahun")->getResultArray();
            ...
            // ORDER BY bulan")->getResultArray();
            $dmo_summary = $db->query("WITH temp_dmo AS (SELECT  qty1, qty2, YEAR(tsco.`bl_date`) AS tahun 
            FROM T_SAL_DMO tsd
            INNER JOIN T_SAL_SHIPMENT tsco ON tsd.contract_no = tsco.contract_no AND tsd.contract_id = tsco.contract_id
            GROUP BY qty1, qty2, tahun)
             SELECT SUM(qty1) AS listrik, SUM(qty2) AS nonlistrik, tahun,
            (SELECT SUM(prd_cg_total) * 0.25 FROM (
                SELECT (CASE WHEN DAY(prd_date) > 25 THEN MONTH(prd_date) + 1 ELSE MONTH(prd_date) END) AS b, prd_cg_total 
                FROM timesheets) a) AS target
            FROM temp_dmo
            GROUP BY tahun")->getResultArray();
            ...
            // Barging (MT) - Chart
            $barging = $builder_shipment->select("SUM(bl_qty) AS total, YEAR(bl_date) AS tahun,
        (SELECT SUM(quantity) FROM T_SAL_TARGET) AS target")
                ->groupBy("tahun")
                ->orderBy("tahun")
                ->get()->getResultArray();
            ...
            // -- versi yearly mobile by ferry
            $prd_sales_prod = $db->query("SELECT SUM(qty/1000) AS total,
            YEAR AS tahun
            FROM T_Adjustment WHERE transaksi = 'Coal Getting'
            GROUP BY tahun")->getResultArray();
            ...
            GROUP BY tahun
            ORDER BY tahun")->getResultArray();
            $prd_hauling = $db->query("SELECT SUM(qty/1000) AS total, YEAR AS tahun
            FROM T_Adjustment WHERE transaksi = 'Hauling to Port'
            GROUP BY tahun ORDER BY tahun")->getResultArray();
            ...
            COALESCE((SELECT SUM(B.ob_monthlybudget_qt) FROM md_monthlybudget B WHERE B.year = $year AND B.month <= $month), 0) AS budget")->getRowArray();
            ...
            JOIN (
                SELECT SUM(B.ob_monthlybudget_qt) AS budget, B.month AS bulan2
                FROM md_monthlybudget B
                 WHERE B.year = '$year'
                GROUP BY bulan2
            ) subquery2
            ...
            JOIN (
                SELECT SUM(B.cg_monthlybudget_qt) AS budget, B.month AS bulan2
                FROM md_monthlybudget B
                 WHERE B.year = '$year'
                GROUP BY bulan2
            ) subquery2
            ...
            (SELECT SUM(actual.qty)
                            ...
                            AND actual.id_contractor = qmaster.id_contractor) AS actual_ob,
            (SELECT IF(MONTH(NOW()) = qmaster.bulan,
			    SUM(budget.ob_dailybudget_qt * DAY(NOW())), 
				SUM(budget.ob_monthlybudget_qt)) AS budget
                            FROM md_monthlybudget budget
                            where budget.year = qmaster.tahun
                            AND budget.month = qmaster.bulan
                            AND budget.id_contractor = qmaster.id_contractor) AS budget_ob
            FROM(
            ...
            (SELECT SUM(actual.qty)/1000
                            ...
                            and actual.month = qmaster.bulan
                                 AND actual.id_contractor = qmaster.id_contractor) AS actual_cg,
             --  //DIBAWAH INI CEK DULU JIKA bulan sekarang maka ambil data cg_dailybudget_qt x tgl hri ini, jika tidak pakai data cg_monthlybudget_qt
            (SELECT IF(MONTH(NOW()) = qmaster.bulan,
			    SUM(budget.cg_dailybudget_qt * DAY(NOW())), 
				SUM(budget.cg_monthlybudget_qt)) AS budget
                            FROM md_monthlybudget budget
                            where budget.year = qmaster.tahun
                            AND budget.month = qmaster.bulan
                            AND budget.id_contractor = qmaster.id_contractor) AS budget_cg
            ...
            (SELECT SUM(actual.qty)/1000
                            ...
                            WHERE actual.transaksi = 'Coal Getting'
                            AND actual.year  = qmaster.tahun
                            and actual.month = qmaster.month) AS actual_cg,
            (SELECT SUM(actual.qty)
                            FROM T_Adjustment actual
                            WHERE actual.transaksi = 'Overburden'
                            AND actual.year  = qmaster.tahun
                            and actual.month = qmaster.month) AS actual_ob,
            (SELECT SUM(budget.cg_monthlybudget_qt)
                            FROM md_monthlybudget budget
                            WHERE budget.year  = qmaster.tahun
                             AND budget.month = qmaster.month) AS budget_cg,
             (SELECT SUM(budget.ob_monthlybudget_qt)
                            FROM md_monthlybudget budget
                            WHERE budget.year  = qmaster.tahun
                            AND budget.month = qmaster.month) AS budget_ob
            FROM(
            SELECT A.year AS tahun, A.month AS month FROM T_Adjustment A WHERE A.transaksi = 'Coal Getting' AND A.year = $year  GROUP BY A.year, A.month
            UNION 
            SELECT A.year AS tahun, A.month AS month FROM T_Adjustment A WHERE A.transaksi = 'Overburden' AND A.year = $year GROUP BY A.year, A.month
            UNION 
            SELECT B.year AS tahun, B.month AS month FROM md_monthlybudget B WHERE B.year = $year GROUP BY B.year, B.month) AS qmaster;")->getResultArray();
            $data['stripping_ratio'] = $stripping_ratio;

            // Sum Overburden Distance & Coal Getting Distance  - Graphic - monhtly
            $data['actual_distance'] = $db->query("WITH t_abc AS (
                SELECT
                YEAR AS tahun, MONTH AS bulan, id_contractor AS contractor, 
                SUM(CASE WHEN transaksi = 'Overburden' THEN qty ELSE 0 END) AS OB,
                SUM(CASE WHEN transaksi = 'Distance OB' THEN qty ELSE 0 END) AS DO,
                SUM(CASE WHEN transaksi = 'Coal Getting' THEN qty/1000 ELSE 0 END) AS CG,
                SUM(CASE WHEN transaksi = 'Distance CG' THEN qty ELSE 0 END) AS DC
                ...
                SUM(OB * DO) / SUM(OB) AS distance_ob,
                SUM(CG * DC) / SUM(CG) AS distance_cg,
               (SELECT SUM(disob_monthlybudget_qty * mm.ob_monthlybudget_qt)/SUM(mm.ob_monthlybudget_qt) FROM md_monthly_disob mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` WHERE mmd.year =  t_abc.tahun AND mmd.month =  t_abc.bulan) AS target_ob,
               (SELECT SUM(discg_monthlybudget_qty * mm.cg_monthlybudget_qt)/SUM(mm.cg_monthlybudget_qt) FROM md_monthly_discg mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` WHERE mmd.year =  t_abc.tahun AND mmd.month = t_abc.bulan) AS target_cg
                FROM t_abc
                WHERE tahun = $year     
            ...
            $contractor_distance = $db->query("WITH t_abc AS (
                ...
                FROM T_Adjustment ta
					 WHERE ta.transaksi = 'Overburden' OR ta.transaksi = 'Distance OB' OR ta.transaksi = 'Coal Getting' OR ta.transaksi = 'Distance CG'
					 AND ta.id_contractor IS NOT NULL  AND YEAR = $year                                                                                                                                                                                                                    
                GROUP BY tahun, bulan, contractor
                )
                SELECT
					 t_abc.tahun, t_abc.bulan, t_abc.contractor AS contractor_id,
               (SELECT tctr.contractor_name from md_contractors tctr WHERE tctr.id = t_abc.contractor) AS contractor,
					 SUM(OB) AS ob_total,
                SUM(CG) AS cg_total,
                SUM(OB * DO) AS ob,
                SUM(CG * DC) AS cg,
                (SELECT SUM(disob_monthlybudget_qty * mm.ob_monthlybudget_qt)/SUM(mm.ob_monthlybudget_qt) FROM md_monthly_disob mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` AND mm.id_contractor = mmd.id_contractor WHERE mmd.year =  t_abc.tahun AND mmd.month =  t_abc.bulan AND mmd.id_contractor = t_abc.contractor) AS target_ob,
                (SELECT SUM(discg_monthlybudget_qty * mm.cg_monthlybudget_qt)/SUM(mm.cg_monthlybudget_qt) FROM md_monthly_discg mmd INNER JOIN md_monthlybudget mm ON mm.`year` = mmd.`year` AND mm.`month` = mmd.`month` AND mm.id_contractor = mmd.id_contractor WHERE mmd.year =  t_abc.tahun AND mmd.month = t_abc.bulan  AND mmd.id_contractor = t_abc.contractor) AS target_cg
                FROM t_abc
                WHERE contractor IS NOT NULL AND tahun = $year
                GROUP BY tahun, bulan, contractor_id
                ORDER BY contractor, tahun, bulan;")->getResultArray();
            $grouped_contracDistance = array();
            foreach ($contractor_distance as $c) {
                $grouped_contracDistance[$c['contractor']][$c['bulan']] = $c;
            ...
            // Sum Crushed Coal (MT)  - chart - monthly - by ferry
            // $sum_cc = $db->query("SELECT qmaster.*,
            // 'OFN' AS crusher,
        ...
        (SELECT SUM(quantity) FROM T_SAL_TARGET WHERE `year` = $year AND `month` = bulan) AS target")
            ...
            // Contractor Performance -  Overburden & Contractor Performance Stripping Ratio  1 - daily same with monthly
            $contractor_ob = $db->query("SELECT qmaster.*,
            (SELECT tctr.contractor_name from md_contractors tctr WHERE tctr.id = qmaster.id_contractor)AS contractor,
            (SELECT SUM(actual.qty)
                            FROM T_Adjustment actual
            ...
            // Contractor Performance - Coal Getting  & Contractor Performance Stripping Ratio  2 - daily same with monthly
            $contractor_cg = $db->query("SELECT qmaster.*,
            (SELECT tctr.contractor_name from md_contractors tctr WHERE tctr.id = qmaster.id_contractor)AS contractor,
            (SELECT SUM(actual.qty)/1000
                            FROM T_Adjustment actual
            ...
            /////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
            //Stripping Ratio Graphic - monhtly same with daily
            $stripping_ratio = $db->query("SELECT qmaster.*,
            (SELECT SUM(actual.qty)/1000
                            FROM T_Adjustment actual
                            WHERE actual.transaksi = 'Coal Getting'
            ...
            // Sum Overburden Distance & Coal Getting Distance  (Per Contractor) - Chart - monthly same with daily
            $contractor_distance = $db->query("WITH t_abc AS (
                SELECT
                YEAR AS tahun, MONTH AS bulan, id_contractor AS contractor, 
                SUM(CASE WHEN transaksi = 'Overburden' THEN qty ELSE 0 END) AS OB,
